...
class GaussianDiffusion(nn.Module):
    ...
    def q_sample(self, x0, t):
        ...
        xt = torch.sqrt(self.alpha_bar[t]) * x0 + torch.sqrt(1 - self.alpha_bar[t]) * noise
        return xt, noise

    def forward(self, img_lr, img_hr, t=None, fix_encoder=False, use_enc_ups=True):
        ...
        #get the residual, 
        #subtract the preliminary upscaled lr image from the hr image
        x0 = self.img2res(x0, img_lr_up)
    ...
    def p_sample(self, lr, scale=4, noise=None, use_enc_ups=True):
        if noise is None:
            ...
            x = torch.randn((b, 1, u*scale*h, v*scale*w))
        else:
            x = noise
