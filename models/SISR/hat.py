...
class OCA(nn.Module):
    ...
    def forward(self, x):
        ...
        x_kv = x_kv.view((-1, c, patch_size, patch_size, hl*wl)).contiguous().permute((0, 4, 2, 3, 1)).contiguous().view((-1, patch_size**2, c))

        #unfold for Q
        x_q = F.unfold(
            x, kernel_size=self.window_size, stride=self.window_size
        )
        x_q = x_q.view((-1, c, self.window_size, self.window_size, hl*wl)).permute((0, 4, 2, 3, 1)).contiguous().view((-1, self.window_size**2, c))
