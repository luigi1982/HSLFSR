...
dataset = LightFieldDataset(data_list, transform=transform)
...
dataset = LightFieldTestDataset(['Fraunhofer'], transform=transform)
test_loader = DataLoader(dataset, batch_size=1, shuffle=False)

### do not add fh as long as it is not properly understood
###test_loaders.append(test_loader)

### Load Model
model_class = MODEL_REGISTRY[cfg.train.model.model]
net = model_class(channels=cfg.train.model.dim).to(DEVICE)
...
for epoch in range(EPOCHS):
    ...
    for i, test_loader in tqdm(enumerate(test_loaders), total=len(test_loaders)):
        ...
        for j, LF in enumerate(test_loader):
            ...
            #SR the Patches
            sub_LF_out = []
            for k in range(0, sub_LF_input.size(0), EVAL_BS):
                tmp = sub_LF_input[k:min(k + EVAL_BS, sub_LF_input.size(0)), :, :, :]
                with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                    with torch.no_grad():
                        net.eval()
